...
pub type BuiltinFuncSign = fn(&mut Runner, &Vec<Value>) -> Value;
...
fn builtin_load_json(runner: &mut Runner, params: &Vec<Value>) -> Value {
...
fn builtin_min(_runner: &mut Runner, params: &Vec<Value>) -> Value {
    let v1 = value::as_f64(params.get(0).unwrap());
    let v2 = value::as_f64(params.get(1).unwrap());
    Value::Num(v1.min(v2))
}

fn builtin_max(_runner: &mut Runner, params: &Vec<Value>) -> Value {
    let v1 = value::as_f64(params.get(0).unwrap());
    let v2 = value::as_f64(params.get(1).unwrap());
    Value::Num(v1.max(v2))
}
